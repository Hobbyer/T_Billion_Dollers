      ...
      <style>
        {`
          @media (max-width: 768px) {
            .write-date, .item-name, .qna-title, .qna-status {
          ...
          <tbody>
            {questions.map((question, i) => {
              ...
              return (
                <tr key={i}>
                  {/* 문의글 작성일 */}
                  <td className="write-date">{dayjs(question.regDate).format("YY-MM-DD HH:mm")}</td>
                  {/* 주문한 상품/상품의 원래 링크 연결 */}
                  ...
                  {/* 문의글에 등록한 제목 */}
                  <td className="qna-title">{question.title}</td>
                  <td className="qna-status">
