...
const TokenRemainButton = () => {
  ...
  const updateRemainingTime = async () => {
    ...
    try {
      const decoded = jwtDecode(token); // 디코딩
      ...
      if (diff <= 0) {
        setRemaining("만료됨");
        await AsyncStorage.removeItem("accessToken"); // 엑세스 토큰 지우기
        await AsyncStorage.removeItem("refreshToken"); // 리프레시 토큰 지우기
        router.navigate("/auth/login"); // 로그인 화면으로 이동
      } else {
        ...
        <View style={styles.overlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalMessage}>
              AccessToken이 성공적으로 갱신되었습니다.
            </Text>
            <Pressable
              style={styles.successBtn}
              onPress={() => setExtendSuccessVisible(false)}
            >
      ...
      <Modal transparent visible={extendErrorVisible} animationType="fade">
        <View style={styles.overlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalMessage}>
              AccessToken 연장에 실패했습니다.
            </Text>
            <Pressable
              style={styles.errorBtn}
              onPress={() => setExtendErrorVisible(false)}
            >
